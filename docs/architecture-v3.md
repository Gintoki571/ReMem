...
- Validation: `Graph::validate` (health: dangling edges + unreviewed edges; SQL failures become report lines, not errors) plus engine `graph_gaps` (ghost/missing nodes) and `suspect_supersedes`; surfaced by CLI `remem validate` (exit 1 on health) and MCP `validate`. Orphan memories are completeness notes via `Graph::orphan_memories` / CLI `validate --show-orphans`, exit 0 (issue #17). Own-API writes cannot dangle (graphqlite FK + cascade); the check targets foreign writers and schema drift.
