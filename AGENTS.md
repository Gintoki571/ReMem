...
- `list [--json]` | `link <from> <to> [--rel r] [--provenance manual|recall-suggested|correction-chain]` (default manual) | `correct <old-id> <kind> <text>` (supersede + correction-chain SUPERSEDES edge) | `forget <id>` | `purge <id>` | `stats` | `validate` (exit 1 if dangling/orphan/unreviewed/suspect-SUPERSEDES) | `related <id>` (id/rel/provenance) | `central` (id/score/provenance-summary) | `path <from> <to>` | `trace <id>` (correction chain, oldest->newest; empty if unknown)` | `supersede <old-id> <kind> <text...>` (soft-supersede + replacement row with `supersedes` back-pointer; prints the new id; errors on unknown/deleted/already-superseded)
