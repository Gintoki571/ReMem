...
`remember(kind, content, tags?, agent?, session?, importance?, occurredAt?)` (occurredAt: unix seconds or `YYYY-MM-DD`), returns `{"id", "similar": [{"id", "distance"}]}` (L2 < 0.48 = near-dup; exact dup returns stored id, empty array),
